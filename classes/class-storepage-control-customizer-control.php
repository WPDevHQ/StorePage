...
class Storepage_Control_Customizer_Control extends WP_Customize_Control {
	...
	public function render_content() {
		if ( ! is_array( $this->choices ) || ! count( $this->choices ) ) {
			printf( __( 'No storepage components found. Please contact your theme author to make your theme compatible with Storepage Control or see this %sguide%s for details on how to do this yourself.', 'storepage-control' ), '<a href="' . esc_url( 'https://docs.woocommerce.com/document/make-your-theme-compatible-with-storepage-control/' ) . '">', '</a>' );
		} else {
				...
				<ul class="storepage-control">
					...
					<?php foreach ( $components as $id => $title ) : ?>
						<?php
							 ...
							 */
							$title = apply_filters( 'storepage_control_title', $title, $id );
